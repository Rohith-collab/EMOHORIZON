...
export const handleAzureChat: RequestHandler = async (req, res) => {
  try {
    ...
    const endpoint = process.env.ENDPOINT_URL || "https://angilambot-api.openai.azure.com/";
    ...
    const messages: AzureChatMessage[] = [
      {
        role: "system",
        content: "You are a helpful AI tutor. Provide clear, educational responses. Keep responses concise and engaging."
      }
    ];

    // Add conversation history if provided
    if (body.conversationHistory && body.conversationHistory.length > 0) {
      messages.push(...body.conversationHistory.map(msg => ({
